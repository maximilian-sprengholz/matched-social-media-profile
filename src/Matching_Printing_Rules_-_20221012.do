...
merge m:1 ID using "C:\Users\geisslef\Dropbox\Covid Vaccine Allocation\4_data\wave_1\D-P21-13185 HU Berlin Impfskeptiker_W1_inkl Device.dta", keepusing(v_144 v_154 v_156)
drop if _merge == 2
drop _merge
foreach var of varlist v_144 v_154 v_156 {
	rename `var' `var'_w1
}

* Merge children from w4 (first refreshment)
merge m:1 ID using "C:\Users\geisslef\Dropbox\Covid Vaccine Allocation\4_data\wave_4\D-P21-13185 HU Berlin Impfskeptiker_W4_inkl Device---Combined.dta", keepusing(v_144 v_154 v_156)
drop if _merge == 2
drop _merge
foreach var of varlist v_144 v_154 v_156 {
	rename `var' `var'_w4
...
* Gen combined v_144
foreach var of varlist v_144 {
	gen `var'_merged = `var'
	mvdecode `var'_merged, mv(-66 -99)
	replace `var'_merged = `var'_w1 if `var'_merged == .
	replace `var'_merged = `var'_w4 if `var'_merged == .
}


* Gen combined v_154
gen v_154_merged = .
replace v_154_merged = v_299
replace v_154_merged = v_154_w1 if v_154_merged == .
replace v_154_merged = v_154_w4 if v_154_merged == .
replace v_154_merged = . if v_154_merged == -99 | v_154_merged == -66


gen v_156_merged = .
replace v_156_merged = v_298
replace v_156_merged = v_156_w1 if v_156_merged == .
replace v_156_merged = v_156_w4 if v_156_merged == .
replace v_156_merged = . if v_156_merged == -99 | v_156_merged == -66
replace v_156_merged = . if v_156_merged == 0
label var v_156_merged "Income in brackets"
label define v_156_merged 0 "" 1 "unter 1000 Euro " 2 "1000 Euro bis unter 2000 Euro" 3 "2000 Euro bis unter 3000 Euro" 4 "3000 Euro bis unter 4000 Euro" 5 "4000 Euro bis unter 5000 Euro" 6 "5000 Euro und mehr"
label val v_156_merged v_156_merged
...
* demography6
cap drop help_otherlanguage
txttool otherlanguage, gen(help_otherlanguage) stopwords("C:\Users\geisslef\_Work\04_Paper\Covid-19-Vaccination\Daten\Wave6\Printing Rules\totlanguage_stopwords.txt")
replace help_otherlanguage = "" if help_otherlanguage == "ansonsten keine andere fremdsprache"
replace help_otherlanguage = "" if help_otherlanguage == "ansonsten andere fremdsprache"

cap drop totlanguage
gen totlanguage = wordcount(help_otherlanguage) + 1
tostring totlanguage, replace
replace totlanguage = "eine" if totlanguage == "1"
replace totlanguage = "zwei" if totlanguage == "2"
replace totlanguage = "drei" if totlanguage == "3"
replace totlanguage = "vier" if totlanguage == "4"
replace totlanguage = "fünf" if totlanguage == "5"
replace totlanguage = "sechs" if totlanguage == "6"
replace totlanguage = "sieben" if totlanguage == "7"
replace totlanguage = "acht" if totlanguage == "8"
replace totlanguage = "neun" if totlanguage == "9"
replace totlanguage = "zehn" if totlanguage == "10"
replace totlanguage = "elf" if totlanguage == "11"
replace totlanguage = "zwölf" if totlanguage == "12"

cap drop demography6
gen str demography6 = ""
replace demography6 = "Insgesamt spreche ich" + " " + totlanguage  + " " + "verschiedene Sprachen" + "."
replace demography6 = "Ich spreche insgesamt" + " " + totlanguage  + " " + "Sprache" + "." if totlanguage == "eine"
...
gen str grownup_ger  = ""
replace grownup_ger  = "Nein" if v_697 == 2
replace grownup_ger  = "Ja" if v_697 == 1
...
cap drop location3
gen str location3 = ""
replace location3 = "Aufgewachsen bin ich in" + " " + print_grownup_ger  + "."
replace location3 = "" if print_grownup_ger  == " "
...
replace secondgen  = "Großeltern" if v_703 == 2
...
cap drop location7
gen str location7 = ""
replace location7 = "Ich wohne im selben Bundesland, in dem ich auch aufgewachsen bin." if samestate == "Ja"
replace location7 = "Ich wohne mittlerweile nicht mehr im selben Bundesland, in dem ich aufgewachsen." if samestate == "Nein"
...
cap drop childrenbracket
gen childrenbracket  = .
replace childrenbracket = 1 if v_144_merged == 0
replace childrenbracket = 2 if v_144_merged == 1 | v_144_merged == 2
replace childrenbracket = 3 if v_144_merged == 3 | v_144_merged == 4
replace childrenbracket = 4 if v_144_merged > 4 & !mi(v_144_merged)
label var childrenbracket "Childrens in brackets"
label define childrenbracket 1 "Keine" 2 "1-2" 3 "3-4" 4 "Mehr als 4"
label val childrenbracket childrenbracket
...
cap drop siblingsbracket 
gen siblingsbracket  = .
replace siblingsbracket = 1 if v_707 == 0
replace siblingsbracket = 2 if v_707 == 1 | v_707 == 2
replace siblingsbracket = 3 if v_707 == 3 | v_707 == 4
replace siblingsbracket = 4 if v_707 > 4 & !mi(v_707)
label var siblingsbracket "Siblings in brackets"
label define siblingsbracket 1 "Keine" 2 "1-2" 3 "3-4" 4 "Mehr als 4"
label val siblingsbracket siblingsbracket
...
replace otherpets = subinstr(otherpets, ";", ",",.) 
...
* finance8
cap drop studentdebt 
gen str studentdebt  = ""
replace studentdebt  = "Ja, und sie sind groß" if v_732 == 1
replace studentdebt  = "Ja, aber sie sind überschaubar" if v_732 == 2
replace studentdebt  = "Nein, ich habe sie abbezahlt" if v_732 == 3
replace studentdebt  = "Nein, ich hatte nie welche" if v_732 == 4

cap drop finance8
gen str finance8 = ""
replace finance8 = "Ob ich Studienschulden habe?" + " " + studentdebt  + "." 
...
replace perfectionist  = "Ansonsten würde ich mich selbst als ziemlichen Perfektionist beschreiben." if v_741 == 1
replace perfectionist  = "Ansonsten würde ich mich selbst nicht als Perfektionist beschreiben." if v_741 == 2

cap drop help_perfectionist 
gen str help_perfectionist  = ""
replace help_perfectionist  = "Ansonsten würde ich mich selbst als ziemlichen Perfektionist beschreiben." if v_741 == 1 & v_16 == 2
replace help_perfectionist  = "Ansonsten würde ich mich selbst als ziemliche Perfektionistin beschreib> en." if v_741 == 1 & v_16 == 1
replace help_perfectionist  = "Ansonsten würde ich mich selbst nicht als Perfektionist beschreiben." if v_741 == 2 & v_16 == 2
replace help_perfectionist  = "Ansonsten würde ich mich selbst nicht als Perfektionistin beschreiben." if v_741 == 2 & v_16 == 1

cap drop personality6
gen str personality6 = ""
replace personality6 = help_perfectionist
...
cap drop smoke 
gen str smoke  = ""
replace smoke  = "Ich bin Raucher." if v_766 == 1
replace smoke  = "Ich bin Raucher, aber nur in Gesellschaft." if v_766 == 2
replace smoke  = "Ich bin Nichtraucher." if v_766 == 3
replace smoke  = "Ich bin Nichtraucher, habe aber früher mal geraucht." if v_766 == 4

cap drop help_smoke 
gen str help_smoke  = ""
replace help_smoke  = "Ich bin Raucher." if v_766 == 1 & v_16 == 2
replace help_smoke  = "Ich bin Raucherin." if v_766 == 1 & v_16 == 1
replace help_smoke  = "Ich bin Raucher, aber nur in Gesellschaft." if v_766 == 2 & v_16 == 2
replace help_smoke  = "Ich bin Raucherin, aber nur in Gesellschaft." if v_766 == 2 & v_16 == 1
replace help_smoke  = "Ich bin Nichtraucher." if v_766 == 3 & v_16 == 2
replace help_smoke  = "Ich bin Nichtraucherin." if v_766 == 3 & v_16 == 1
replace help_smoke  = "Ich bin Nichtraucher, habe aber früher mal geraucht." if v_766 == 4 & v_16 == 2
replace help_smoke  = "Ich bin Nichtraucherin, habe aber früher mal geraucht." if v_766 == 4 & v_16 == 1

cap drop thingsyoudo2
gen str thingsyoudo2 = ""
replace thingsyoudo2 = help_smoke 
...
replace thingsilike6 = substr(thingsilike6, 1, length(thingsilike6) - 1) if substr(thingsilike6, -1, 1) ==  " "
replace thingsilike6 = substr(thingsilike6, 1, length(thingsilike6) - 1) if substr(thingsilike6, -1, 1) ==  ";"
...
* currentzip
cap drop currentzip
clonevar currentzip = v_18
destring currentzip, replace
...
* hometown_foreign
cap drop hometown_foreign
clonevar hometown_foreign = v_700
replace hometown_foreign = ustrtitle(hometown_foreign)
replace hometown_foreign = "" if v_697 == 1
...
cap drop samezip
gen str samezip = ""
replace samezip = "Ja" if currentzip == homezip_ger
replace samezip = "Nein" if currentzip != homezip_ger & !mi(currentzip) & !mi(homezip_ger)
