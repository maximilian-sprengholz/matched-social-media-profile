  ...
  with_progress({
    ...
    df_match_all <- df %>%
      ...
      future_pmap_dfr(function(...) {
        ...
        # dataframe containing all others which can be used in matching, shuffled
        ...
        df_match_person <- df_p2 %>%
          pmap_dfr(function(...) {
            ...
            if (any(nmatches[[opinion_matched]] < maxmatches)) {
              ...
              # sum scores, determine if return matches still needed in group
              ...
              if (simscore <= simlow & nmatches[[opinion_matched]]$simlow < maxmatches) {
                ...
                # write considerations count when maxmatches reached in group full
        ...
        } else if (nrow(df_match_person) > 0) {
          df_match_opsame_simlow <- df_match_person %>%
            filter(essay_opinion_prior_matched == 1) %>%
            slice(which.min(match_simscore)) %>%
            mutate(match_group = "Same opinion, different characteristics")
        } else {
          df_match_opsame_simlow <- data.frame() # fallback: no matches possible
        }
        ...
        if (nmatches$opsame$simhigh > 0) {
          ...
          df_match_opsame_simhigh <- data.frame() # fallback: no matches possible
        }
        ...
        if (nmatches$opdiff$simlow > 0) {
          ...
          df_match_opdiff_simlow <- data.frame() # fallback: no matches possible
        }
        ...
        if (nmatches$opdiff$simhigh > 0) {
          ...
          df_match_opdiff_simhigh <- data.frame() # fallback: no matches possible
        }
        
        # bind together
        df_match_person <- bind_rows(
...
write_rds(df_matches, paste0(wd, "/data/matches.rds"))
